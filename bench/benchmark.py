...
def base_fitness(Population, func, intercept):
    ...
    for indi in Population:
        fitness.append(func(indi) * np.random.normal(loc=0, scale=0.01, size=None) - intercept)
    return fitness
...
def groups_fitness(groups, Population, func, cost, intercept):
    ...
    for indi in Population:
        indi_fitness = 0
        for group in groups:
            indi_fitness += (func(group_individual(group, indi)) * np.random.normal(loc=0, scale=0.01, size=None) -
                             intercept)
            cost += 1
